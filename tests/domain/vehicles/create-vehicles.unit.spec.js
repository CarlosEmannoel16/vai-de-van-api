...
const CreateVechicle_1 = require("@/data/usecases/vechicle/CreateVechicle");
const makerVehicleRepository = () => {
    return {
        associate: jest.fn(),
...
describe('Unit Create Vehicle Use Case', () => {
    test('Deve retornar erro se placa ja estiver cadastrada', async () => {
        ...
        const sut = new CreateVechicle_1.CreateVehicleUseCase(mockMakerVehicleRepository, mockMakerUserRepository);
    ...
    test('Deve retornar erro se proprietario nao existir', async () => {
        ...
        await expect(sut.execute(input)).rejects.toThrow('Proprietário não encontrado');
    });
});
